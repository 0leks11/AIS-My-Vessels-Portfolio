    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center text-sm text-gray-600 mt-1">
          <Flag code={vessel.flagCode} className="w-8 h-8 mr-2" />
          <span>{vessel.flag}</span>
        </div>
        <div className="relative">
          {vessel.imageComponent && <vessel.imageComponent />} {/* Рендерим компонент изображения */}
          <div className="absolute top-2 left-2 bg-white text-xs font-bold rounded-md shadow">
            {vessel.type}
          </div>
        </div>
